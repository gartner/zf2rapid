...
/**
 * Class LoadControllerPlugins
 *
 * @package ZF2rapid\Task\Fetch
 */
class LoadControllerPlugins extends AbstractTask
{
    /**
     * Process the command
     *
    ...
    public function processCommandTask()
    {
        ...
        $loadedControllerPlugins = array();

        // loop through loaded modules and fetch controllers for module
        foreach ($this->params->loadedModules as $moduleName => $moduleObject) {
            $loadedControllerPlugins[$moduleName] = $this->loadPluginsForModule(
                $moduleObject
            );
        }

        // set loaded plugin
        $this->params->loadedControllerPlugins = $loadedControllerPlugins;

        // check loaded modules
        if (!empty($this->params->loadedControllerPlugins)) {
            return 0;
        }
    ...
    /**
     * Get controller plugins for a module
     *
     * @param $moduleObject
     *
     * @return array
