 ...
 */
class LoadFilters extends AbstractTask
{
    /**
     * Process the command
     *
    ...
    public function processCommandTask()
    {
        ...
        foreach ($this->params->loadedModules as $moduleName => $moduleObject) {
            $loadedFilters[$moduleName] = $this->loadPluginsForModule(
                $moduleObject
            );
        }

        // set loaded plugin
        $this->params->loadedFilters = $loadedFilters;

        // check loaded modules
        if (!empty($this->params->loadedFilters)) {
            return 0;
        }
