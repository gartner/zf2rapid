 ...
 */
class LoadForms extends AbstractTask
{
    /**
     * Process the command
     *
    ...
    public function processCommandTask()
    {
        ...
        foreach ($this->params->loadedModules as $moduleName => $moduleObject) {
            $loadedForms[$moduleName] = $this->loadPluginsForModule(
                $moduleObject
            );
        }

        // set loaded plugin
        $this->params->loadedForms = $loadedForms;

        // check loaded modules
        if (!empty($this->params->loadedForms)) {
            return 0;
        }
