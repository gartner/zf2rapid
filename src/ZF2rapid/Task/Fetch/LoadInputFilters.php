...
/**
 * Class LoadInputFilters
 *
 * @package ZF2rapid\Task\Fetch
 */
class LoadInputFilters extends AbstractTask
{
    /**
     * Process the command
     *
    ...
    public function processCommandTask()
    {
        ...
        $loadedInputFilters = array();

        // loop through loaded modules and fetch controllers for module
        foreach ($this->params->loadedModules as $moduleName => $moduleObject) {
            $loadedInputFilters[$moduleName] = $this->loadPluginsForModule(
                $moduleObject
            );
        }

        // set loaded plugin
        $this->params->loadedInputFilters = $loadedInputFilters;

        // check loaded modules
        if (!empty($this->params->loadedInputFilters)) {
            return 0;
        }

        $this->console->writeTaskLine(
            'task_fetch_load_input_filters_not_found',
            array(
    ...
    /**
     * Get input filters for a module
     *
     * @param $moduleObject
     *
     * @return array
